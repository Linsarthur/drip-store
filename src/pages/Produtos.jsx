...
const Produtos = () => {
  ...
  useEffect(() => {
    if (filtroMarca.length > 0) {
      setProdutosFiltrados([
        ...produtos.filter((produto) => filtroMarca.includes(produto.marca)),
      ]);
      return;
    }
  ...
  useEffect(() => {
    if (filtroGenero.length > 0) {
      setProdutosFiltrados([
        ...produtos.filter((produto) => filtroGenero.includes(produto.genero)),
      ]);
      return;
    }
    setProdutosFiltrados([...produtos]);
  }, [produtos, filtroGenero]);

  useEffect(() => {
    if (filtroCategoria.length > 0) {
      setProdutosFiltrados([
        ...produtos.filter((produto) =>
          filtroCategoria.includes(produto.categoria)
        ),
      ]);
      return;
    }
  ...
  useEffect(() => {
    if (filtroEstado) {
      setProdutosFiltrados([
        ...produtos.filter((produto) => filtroEstado.includes(produto.estado)),
      ]);
      return;
    }
    setProdutosFiltrados([...produtos]);
  }, [produtos, filtroEstado]);
      ...
      <div className="flex gap-[28px] items-start mt-[40px]">
        ...
        <div className="grid grid-cols-3 gap-[14px]">
          {produtosFiltrados.length > 0 &&
            produtosFiltrados.map((produto) => <Produto {...produto} />)}
        </div>
      </div>
    </div>
  );
